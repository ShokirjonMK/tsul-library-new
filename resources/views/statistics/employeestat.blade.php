                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-hover">
                                ...
                                @foreach ($usersEmps as $usersEmp)
                                    <tr>
                                        <td>
                                            <a href="{{ route('users.show', [app()->getLocale(),  $usersEmp->id ]) }}"> {{ $usersEmp->id }} </a>
                                        </td>
                                        <td>
                                            <a href="{{ route('users.show', [app()->getLocale(),  $usersEmp->id ]) }}">
                                                ...
                                                @if(($usersEmp->inventar_number))
                                                    ...
                                                    @if (env('USER_BAR_CODE_TYPE')=='QRCODE')
                                                        {!! QrCode::size(100)->generate($usersEmp->inventar_number); !!}
                                                    @else
                                                        @php
                                                            $generator = new Picqer\Barcode\BarcodeGeneratorSVG();
                                                            echo $generator->getBarcode($usersEmp->inventar_number, $generator::TYPE_CODE_128, 2.30);
                                                        @endphp
                                                    @endif
                                                @endif
                                        ...
                                        <td>
                                            ...
                                            <b>{!! \App\Models\Book::GetCountBookByUserByTwoMonth($usersEmp->id, $from, $to)!!}</b><br>
                                            {{__('Those attached to the branch')}}:
                                            <b>{!! \App\Models\BookInformation::GetCountBookInformationByUserByTwoMonth($usersEmp->id, $from, $to)!!}</b>
                                            <br>
                                            {{ __('In copy') }}:
                                            <b>{!! \App\Models\BookInventar::GetCountBookCopyByUserByTwoMonth($usersEmp->id, $from, $to)!!}</b>
                                        </td>
                                        <td>
                                            {{__('Issue of books')}}:
                                            <b>{!! \App\Models\Debtor::GetCountBookByUserByTwoMonth($usersEmp->id, $from, $to, \App\Models\Debtor::$GIVEN)!!}</b>
                                            <br>
                                            {{__('Number of books count')}}:
                                            <b>{!! \App\Models\Debtor::GetCountBookByUserByTwoMonth($usersEmp->id, $from, $to, \App\Models\Debtor::$TAKEN)!!}</b>
                                        </td>
                                        <td>
